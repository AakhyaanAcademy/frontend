...
function Subjects(props) {
    ...
    return (
        <div className={`w-full h-screen bg-back pt-20 dark:bg-background`}>
            <Head>
                <title>{`${data?.data?.course?.title ? data?.data?.course?.title + " - " : ""}Submissions - Chapterwise Test | Aakhyaan Academy`}</title>
            ...
            <Breadcrumb props={[{ header: "Submissions", link: "profile/submissions/chapterwise-test" }, { header: `${router.query.program}`, link: `profile/submissions/chapterwise-test/${router.query.courseId}?program=${router.query.program}&programId=${router.query.programId}` }]} />
                        ...
                        <>
                            {subjects ?
                                subjects?.map((subject, i) =>
                                    <Link
                                        key={i}
                                        href={`${router.query.courseId}/${subject._id}?program=${router.query.program}&programId=${router.query.programId}&subject=${subject.title}`}
                                        legacyBehavior>
