...
function Submission() {
    ...
    const { data, error } = useSWR(id ? process.env.BACKEND + `/mcq/submission/${id}` : null, async url => await getSolvedMcq(url), { revalidateOnFocus: false, revalidateOnReconnect: true })
    ...
    return (
        <div className="h-screen">
            <Head>
                <title>{`${data?.data?.submission?.mcqTitle ? data?.data?.submission?.mcqTitle + " - " : ""}Submissions - Mock Test | Aakhyaan Academy`}</title>
            ...
            {error ?
                <div className={`pt-[60px] md:pt-[80px] relative dark:bg-background dark:text-mainText w-full min-h-full flex flex-col justify-center items-center`}>
                    ...
                    <div onClick={() => router.push(`/profile/submissions/mock-test/${router.query.courseId}?program=${router.query.program}&programId=${router.query.programId}`)} className="cursor-pointer font-sans text-lg transition-all bg-green-800 text-white py-1 mt-4 px-3 rounded focus:outline-none focus:shadow-outline hover:translate-y-[1px]">
                        Go Back
                    </div>
                </div>
                :
