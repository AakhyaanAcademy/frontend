...
function Programmes(props) {
  ...
  return (
    <div className={`w-full h-screen bg-back pt-20 dark:bg-background`}>
      <Head>
        <title>Programmes | Aakhyaan Academy</title>
        <meta
          name="description"
          content="We cover variety of courses and provide free contents according to the needs, along with some premium contents."
        />
        <meta
      ...
      <Breadcrumb props={[{ header: "Programmes", link: "programmes" }]} />
            ...
            <>
              {programmes ?
                programmes.map((program, i) =>
                  <Link
                    key={i}
                    href={`/programmes/details/${program._id}?program=${program.title}&programId=${program._id}`}
                    legacyBehavior>
                    <a className={`max-w-sm lg:max-w-full flex cursor-pointer hover:translate-y-1 justify-center`} >
                      ...
                      <div className="relative shadow-xl min-h-[200px] md:min-h-[220px] w-[160px] md:w-[240px] border-transparent border-gray-400 dark:bg-gray-800 bg-back dark:text-mainText/80 rounded-md p-8 flex flex-col justify-between">
                        <div className="my-auto flex flex-col items-center justify-center">
                          <div className="absolute bottom-2 right-2 flex gap-2 text-sm items-center">
                            {program?.price ?
                              <div className="text-yellow-900 dark:text-[#FFD700]">Rs.&nbsp;{program?.price}</div>
                              :
                              <>
