...
function Chapters(props) {
    ...
    return (
        <div className={`w-full h-screen bg-back pt-20 dark:bg-background`}>
            <Head>
            <title>{`${data?.data?.subject?.title ? data?.data?.subject?.title + " - " : ""}${data?.data?.course?.title ? data?.data?.course?.title + " - " : ""}Chapterwise Test | Aakhyaan Academy`}</title>
                <meta name="keywords" content={`${data?.data?.course?.title}`} />
                <meta property="og:title" content={`${data?.data?.course?.title ? data?.data?.course?.title + " - " : ""}Chapterwise Test | Aakhyaan Academy`} key="ogtitle" />
                <meta property="og:description" content="Take your own test online and prepare yourself. We provide regular chapterwise tests for everyone who are the members of our Academy." key="ogdesc" />
            </Head>
            <Header page="programmes" />

            <Breadcrumb props={[{ header: "Test ", link: "mcqs" },{ header: `${router.query.program}`, link: `mcqs/${router.query.courseId}?program=${router.query.program}&programId=${router.query.programId}` },{ header: `Chapterwise Test`, link: `mcqs/${router.query.courseId}/chapterwise-test?program=${router.query.program}&programId=${router.query.programId}` },{ header: `${router.query.subject}`, link: `mcqs/${id1}/chapterwise-test/${id2}?program=${router.query.program}&programId=${router.query.programId}&subject=${router.query.subject}` }]} />
                        ...
                        <>
                            {chapters ?
                                chapters?.map((chapter, i) =>
                                    <Link
                                        key={i}
                                        href={`/mcqs/${id1}/chapterwise-test/${id2}/${chapter._id}/?program=${router.query.program}&programId=${router.query.programId}&subject=${router.query.subject}&chapter=${chapter.title}`}
                                        legacyBehavior>
