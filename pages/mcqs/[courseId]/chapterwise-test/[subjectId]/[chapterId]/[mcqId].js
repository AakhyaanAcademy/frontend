...
export default function Mcq() {
    ...
    const id = router.query.mcqId
    let answered
    const [questions, setQuestions] = useState([])
    const questionsRef = useRef([])
    const [preCheckTimeRemaining, setPreCheckTimeRemaining] = useState(null)

    const { data: preCheckData, error: preCheckError } = useSWR(!start && id ? [process.env.BACKEND + `/chapterMcq/precheck`, id] : null, async url => {
        return await preCheckMcq(url, { mcqId: id });
    }, { revalidateOnFocus: false, revalidateOnReconnect: false })

    const { data, error } = useSWR(start && !submitted && id ? process.env.BACKEND + `/chapterMcq/start` : null, async url => {
    ...
    const saveState = async (e) => {
        e?.preventDefault()
        try {
            answered = await saveMcq(process.env.BACKEND + "/chapterMcq/save", { answers: answers, mcqId: id })
    ...
    const handleSubmit = async (e) => {
        e?.preventDefault()
        setSubmitted(true)
        answered = await submitMcq(process.env.BACKEND + "/chapterMcq/submit", { answers: answers, mcqId: id })
    }
    ...
    return (
        <div className='h-screen'>
            <Head>
                <title>{`${title ? title + " - " : ""}Chapterwise Test | Aakhyaan Academy`}</title>
                <meta
                    name="description"
                    content="Start your Mcq session with just a click."
                />
                <meta property="og:title" content={`${title ? title + " - " : ""}Chapterwise Test | Aakhyaan Academy`} key="ogtitle" />
            ...
            {start ?
                        ...
                        <>
                            ...
                            <div className={`pt-[60px] md:pt-[80px] relative dark:bg-background dark:text-mainText w-full min-h-full flex flex-col overflow-hidden`}>
                                ...
                                <div className='flex flex-col gap-10 xl:flex-row xl:gap-5'>
                                    <div className={`m-auto xl:hidden mt-10`}>
                                    ...
                                    <div className='gap-10 dark:bg-background bg-back h-full flex flex-col p-10'>
                                        {displayQuestions}
                                    </div>
                            ...
                            <div className="dark:text-mainText/90 border-transparent px-6 py-8 md:px-7 md:py-12 dark:bg-formBox bg-purple-900 shadow-md rounded-md flex flex-col gap-5">
                                {submitted ?
                                    <>
                                        ...
                                        <div className='mt-2'>Please go to the <span className='underline'><Link href={`/profile/submissions/chapterwise-test/${router.query.courseId}/${router.query.subjectId}/${router.query.chapterId}?program=${router.query.program}&programId=${router.query.programId}&subject=${router.query.subject}&chapter=${router.query.chapter}`} >Submissions</Link></span> page to see your result of past mcqs.</div>
                                    </>
                                    :
                                    <>
                                        {error ?
                                ...
                                <div className="">
                                    <span className="float-right flex gap-5">
                                        <div onClick={() => router.push(`/mcqs/${router.query.courseId}/chapterwise-test/${router.query.subjectId}/${router.query.chapterId}?program=${router.query.program}&programId=${router.query.programId}&subject=${router.query.subject}&chapter=${router.query.chapter}`)} className="cursor-pointer font-sans text-lg transition-all bg-green-800 text-white py-2 px-4 rounded focus:outline-none focus:shadow-outline hover:translate-y-[1px]">
                                            Go Back
                                        </div>
                                    </span>
                                </div>
                ...
                <div className="bg-gray-500 dark:bg-background/80 p-[6%] pt-[5%] flex items-center justify-center text-lg font-normal pb-10 min-h-full">
                    <div className="dark:text-mainText/90 border-transparent p-8 dark:bg-formBox bg-gray-600 shadow-md rounded-md flex flex-col gap-8">
                        ...
                        {preCheckError ?
                            preCheckError?.response?.data?.data?.mcqId ?
                                <div className='flex flex-col gap-1 text-blue-300'>
                                    <Link href={`/mcqs/${router.query.courseId}/chapterwise-test/${router.query.subjectId}/${router.query.chapterId}/${preCheckError?.response?.data?.data?.mcqId}?mcq=${preCheckError?.response?.data?.data?.mcqTitle}&program=${router.query.program}&programId=${router.query.programId}&subject=${router.query.subject}&chapter=${router.query.chapter}`}>Click here to go there.</Link>
                                </div>
                                :
                                <></>
                            :
                        ...
                        <div>
                            <span className="float-right flex gap-5">
                                <button onClick={() => router.push(`/mcqs/${router.query.courseId}/chapterwise-test/${router.query.subjectId}/${router.query.chapterId}?program=${router.query.program}&programId=${router.query.programId}&subject=${router.query.subject}&chapter=${router.query.chapter}`)} className="font-sans text-lg transition-all bg-red-900 text-white py-2 px-4 rounded focus:outline-none focus:shadow-outline hover:translate-y-[1px]">
                                    Back
                                </button>
...
export const NavigationBox = ({ questions, answers, questionsRef, currentPage, setCurrentPage, setActivePage }) => {
    return (
        <div className='h-fit mt-10 flex flex-col gap-5'>
            <div className='dark:bg-formBox bg-slate-100 border-2 flex flex-col divide-y-2 text-sm'>
                    ...
                    <div key={i} className='flex divide-x-2 cursor-pointer'>
                        {[...Array(10)].map((arr, j) =>
                            i * 10 + j < questions?.length ?
                                ...
                                }} key={i * 10 + j} className={`w-8 text-center p-1 ${answers[i * 10 + j] !== null ? "bg-testColor/70" : ""}`}>{i * 10 + j + 1}</div>
                                :
                                <></>
                        )}
                    </div>
